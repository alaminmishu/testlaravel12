...
`where('environment', 'mongo')`).

| Widget | Chart | What it shows |
|---|---|---|
| `RevenueStats` | Stat cards | Total revenue, total orders, average order value, and unique customers (distinct `customer_email`) — all-time. |
| `RevenuePerMonthChart` | Bar | `SUM(total_amount)` grouped by month, trailing 12 months. |
| `OrdersPerMonthChart` | Horizontal bar | Order count grouped by month, trailing 12 months. |
| `OrdersStatusDonut` | Donut | Order count grouped by `status` (DELIVERED, PROCESSING, CANCELLED, …). |
| `TotalCustomersChart` | Line | Distinct `customer_email` count per month, trailing 12 months — i.e. monthly active customers, not cumulative total. |
...
- **No date-range filtering** — every widget is hardcoded to "last 12
  months" or "all time"; there's no way to pick a custom window from the UI.
- **No caching** — every dashboard load re-runs all aggregate queries
  against the full `orders`/`order_items` tables. Fine at 140k orders; will
  need attention as the table grows.
...
2. **Dashboard filters** — add a date-range picker (Filament supports
   per-page filters) so widgets aren't locked to a fixed trailing window.
3. **Caching layer** — wrap widget queries in `Cache::remember()` with a
   short TTL (a few minutes), invalidated after each `orders:sync-mongo` run,
   to avoid re-scanning the full tables on every dashboard visit.
4. **Sync observability** — alert (email/Slack) on `orders:sync-mongo`
   failure, and expose last-sync time / row counts somewhere in the admin
   panel (e.g. a small widget reading `sync_states`).
5. **Decide the fate of the GraphQL pipeline** — either retire
...
6. **Telescope hygiene** — schedule `telescope:prune` regularly; the
   disk-fill incident during backfill showed the entries table grows fast
   under normal (non-bulk) usage too.
7. **Widget test coverage** — current tests cover the mapper and sync
   command only; add tests asserting each widget's query returns the
   expected shape given seeded `Order`/`OrderItem` factory data.
8. **Replace the temporary admin user** — `admin@example.com` /
   `password` was created only to verify the dashboard renders; swap for
   real authentication before this goes anywhere shared.
9. **Data retention** — `orders`/`order_items` will keep growing with every
